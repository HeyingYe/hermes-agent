...
- Missing/false dispatch_in_gateway exits before opening any DB.
- HERMES_KANBAN_DISPATCH_IN_GATEWAY env var can disable without loading config.
...
def test_notifier_watcher_skips_when_dispatch_missing():
    """Missing dispatch_in_gateway keeps generic gateways out of Kanban DBs."""
    runner = _make_runner()
    with patch("hermes_cli.config.load_config", return_value={"kanban": {}}):
        with patch("hermes_cli.kanban_db.connect") as mock_connect:
            asyncio.run(_RunnerBackedKanbanWatchers(runner)._kanban_notifier_watcher())
...
def test_notifier_watcher_env_override_enables(monkeypatch):
    """HERMES_KANBAN_DISPATCH_IN_GATEWAY=true enables without config opt-in."""
    ...
    mock_load_config.assert_not_called()
    assert past_gate, "env opt-in should let notifier reach board fan-out"
