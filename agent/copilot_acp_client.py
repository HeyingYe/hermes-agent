...
# ── T2b persistent-session helpers ───────────────────────────────────────────
# A persistent ACP session holds the conversation state server-side, so each turn
# we send only the *new* non-assistant messages (assistant turns are generated by
# the session itself and are never resent). These helpers key a conversation, hash
# its inputs for divergence detection, and render an incremental delta WITHOUT the
# system preamble/tools (already sent on the session's first prompt).

def _conversation_fingerprint(messages: list[dict[str, Any]] | None) -> str | None:
    """Stable key for a Hermes conversation = sha1(first system + first user content).

    Self-contained (no session-id plumbing). If compression later rewrites the
    opening, the fingerprint changes → a new persistent session (old one idles
    out) → still correct. Returns ``None`` when there is no user message to key on.
    """
    if not messages:
        return None
    sys_txt = ""
    ...
    for m in messages:
        ...
        if role == "system" and not sys_txt:
            sys_txt = _render_message_content(m.get("content"))
        elif role == "user" and not usr_txt:
            usr_txt = _render_message_content(m.get("content"))
            break
    if not usr_txt:
        return None
    return hashlib.sha1((sys_txt + "\x00" + usr_txt).encode("utf-8", "replace")).hexdigest()


def _nonassistant_signature(messages: list[dict[str, Any]] | None) -> list[tuple[int, str]]:
    """``[(index, content_hash), ...]`` for non-assistant messages (system/user/tool)
    — exactly the inputs an ACP session must receive. Assistant messages are skipped
    (the session generated them). Used both to detect prefix divergence and to slice
    out the incremental delta to send."""
    ...
    for i, m in enumerate(messages or []):
        ...
        if role == "assistant":
            continue
        h = hashlib.sha1(
            (role + "\x00" + _render_message_content(m.get("content"))).encode("utf-8", "replace")
        ).hexdigest()
        sig.append((i, h))
    return sig


def _format_delta_as_prompt(new_messages: list[dict[str, Any]]) -> str:
    """Render only the new turns as a transcript, WITHOUT the system preamble/tools
    (the persistent session already has them). Mirrors the transcript labels used by
    :func:`_format_messages_as_prompt` so the session sees a consistent format."""
    transcript: list[str] = []
    for message in new_messages:
        ...
        transcript.append(f"{label}:\n{rendered}")
    body = "\n\n".join(transcript)
    return (body + "\n\nContinue the conversation from the latest user request.").strip()


def _is_prefix(short: list[str], full: list[str]) -> bool:
    """True iff ``short`` is a (proper or equal) prefix of ``full``."""
...
def _acp_feature_flags() -> tuple[bool, bool]:
    """``(persistent_session, disable_builtin_tools)`` from ``route_decision`` config.

    * ``acp_persistent_session`` (T2b): reuse a persistent ACP session per Hermes
      conversation + send only incremental turns (prefix-cache the history).
    * ``acp_disable_builtin_tools``: send ``_meta.disableBuiltInTools`` so the engine
      drops its own tools (Hermes drives tools itself; shrinks the system prefix).

    Both default ``False`` → unchanged behavior. Read via the gateway-safe
    mtime-cached ``load_config_readonly`` (cheap per call).
    """
    try:
        from hermes_cli.config import load_config_readonly
        ...
        rd = (load_config_readonly() or {}).get("route_decision", {}) or {}
        return (
            bool(rd.get("acp_persistent_session", False)),
            bool(rd.get("acp_disable_builtin_tools", False)),
        )
    except Exception:
        return False, False
...
class _T2bSession:
    """A persistent ACP session bound to one Hermes conversation (T2b).

    Holds a *dedicated* connection (NOT the T2a pool — the session state lives in
    that specific process), the live ACP ``sessionId``, and the hashes of the
    non-assistant messages already sent (for prefix-divergence detection). Its
    ``lock`` serializes the conversation's turns.
    """

    def __init__(self) -> None:
        self.conn: _AcpConnection | None = None
...
class _T2bRegistry:
    """Module-level map ``(command, args, cwd, fingerprint) -> _T2bSession`` (T2b).

    Lives at module scope for the same reason as :class:`_AcpPool`: the per-request
    client is closed after every call, so persistent session state can't live on it.
    Lazy idle/dead reaping on acquire + ``atexit`` so we never leak processes.
    """

    def __init__(self) -> None:
        self._by_key: dict[tuple, _T2bSession] = {}
        self._lock = threading.Lock()

    @staticmethod
    def _key(command: str, args: list[str], cwd: str, fp: str) -> tuple:
        return (command, tuple(args), cwd, fp)

    def acquire(self, command: str, args: list[str], cwd: str, fp: str, idle_timeout: float) -> _T2bSession:
        key = self._key(command, args, cwd, fp)
        now = time.monotonic()
        with self._lock:
            # Reap *other* dead/idle-expired entries to bound the registry.
            for k, s in list(self._by_key.items()):
                if k == key:
                    continue
                if s.conn is None or not s.conn.alive() or (now - s.last_used) > idle_timeout:
                    s.reset()
                    del self._by_key[k]
...
_T2B = _T2bRegistry()
atexit.register(_T2B.close_all)
...
class CopilotACPClient:
    ...
    def _create_chat_completion(
        ...
        **_: Any,
        ...
        persistent_session, disable_tools = _acp_feature_flags()
        ...
        if persistent_session and model and messages:
            # T2b: reuse a persistent ACP session + send only the incremental turn.
            # Falls back to a fresh full-history session internally on divergence,
            # dead session, or unkeyable conversation — output stays correct.
            response_text, reasoning_text, acp_usage = self._run_persistent(
                messages, model, tools, tool_choice, _effective_timeout, disable_tools,
            )
        else:
        ...
        *,
        ...
        if persistent:
            ...
            with conn.lock:
                with self._active_process_lock:
                    ...
                    self.is_closed = False
                try:
                    result = self._run_on_connection(conn, prompt_text, timeout_seconds, model, disable_tools)
                except Exception:
        ...
        try:
            ...
            with self._active_process_lock:
                self._active_process = conn.proc
                self.is_closed = False
            return self._run_on_connection(conn, prompt_text, timeout_seconds, model, disable_tools)
        finally:
            conn.close()
            with self._active_process_lock:
                self._active_process = None
    ...
    def _run_persistent(
        ...
        """T2b: keep one ACP session per Hermes conversation and send only the new
        non-assistant turns, so Claude's prefix cache covers the history.

        Correctness over cache, always: if the conversation can't be keyed, the
        prior-sent prefix diverges (compression/edit), or the session/process is
        dead, we fall back to a fresh ``session/new`` with the full transcript —
        exactly the non-persistent behavior — and never cross-talk between turns.
        """
        _proc, _psize, idle_timeout = _acp_persistence_settings()
        fp = _conversation_fingerprint(messages)
        sig = _nonassistant_signature(messages)
        cur_hashes = [h for _, h in sig]

        if not fp or not sig:
            # Unkeyable conversation → plain full send (no persistence).
            prompt_text = _format_messages_as_prompt(
                messages, model=model, tools=tools, tool_choice=tool_choice
            )
            return self._run_prompt(
                prompt_text, timeout_seconds=timeout_seconds, model=model, disable_tools=disable_tools
            )

        entry = _T2B.acquire(self._acp_command, self._acp_args, self._acp_cwd, fp, idle_timeout)
        with entry.lock:
            reuse = (
                entry.conn is not None
                and entry.conn.alive()
                and bool(entry.session_id)
                and _is_prefix(entry.sent_hashes, cur_hashes)
                and len(cur_hashes) > len(entry.sent_hashes)  # there is a delta to send
            )
            try:
                if reuse:
                    new_msgs = [messages[i] for i, _ in sig[len(entry.sent_hashes):]]
                    delta_text = _format_delta_as_prompt(new_msgs)
                    with self._active_process_lock:
                        self._active_process = entry.conn.proc
                        self.is_closed = False
                    self._drain(entry.conn)
                    result = self._send_prompt(
                        entry.conn, entry.session_id, delta_text, timeout_seconds, disable_tools
                    )
                    entry.sent_hashes = cur_hashes
                    entry.last_used = time.monotonic()
                    logger.info(
                        "ACP T2b: incremental turn on persistent session (%d new msg(s), sig=%d)",
                        len(new_msgs), len(cur_hashes),
                    )
                    return result

                # Fresh (re)start: dedicated connection + session/new + full history.
                entry.reset()
                entry.conn = _AcpConnection(self._acp_command, self._acp_args, self._acp_cwd)
                entry.conn.start()
                with self._active_process_lock:
                    self._active_process = entry.conn.proc
                    self.is_closed = False
                self._ensure_initialized(entry.conn, timeout_seconds)
                entry.session_id = self._open_new_session(
                    entry.conn, model, timeout_seconds, disable_tools
                )
                full_text = _format_messages_as_prompt(
                    messages, model=model, tools=tools, tool_choice=tool_choice
                )
                result = self._send_prompt(
                    entry.conn, entry.session_id, full_text, timeout_seconds, disable_tools
                )
                entry.sent_hashes = cur_hashes
                entry.last_used = time.monotonic()
                logger.info("ACP T2b: fresh persistent session (full history, sig=%d)", len(cur_hashes))
                return result
            except Exception:
                entry.reset()  # poison a broken session — never reuse it
                raise
            finally:
                with self._active_process_lock:
                    self._active_process = None
    ...
    def _run_on_connection(
        ...
        """Run one ACP prompt cycle on ``conn`` against a FRESH session: ensure
        initialize (once per connection), open a new session, bind the model, send
        the prompt. A fresh ``session/new`` per call keeps output identical to the
        per-call-spawn path — only spawn + ``initialize`` cold start is amortized
        when the connection is reused (T2a). T2b's persistent path reuses
        ``_send_prompt`` directly on an existing session instead."""
        if conn.proc is None:
            conn.start()
            with self._active_process_lock:
                self._active_process = conn.proc
        self._drain(conn)
        self._ensure_initialized(conn, timeout_seconds)
        session_id = self._open_new_session(conn, model, timeout_seconds, disable_tools)
        return self._send_prompt(conn, session_id, prompt_text, timeout_seconds, disable_tools)

    def _drain(self, conn: "_AcpConnection") -> None:
        """Drain trailing messages from a prior prompt on a reused connection so
        late notifications can't bleed into this prompt."""
        while True:
            try:
    ...
    def _ensure_initialized(self, conn: "_AcpConnection", timeout_seconds: float) -> None:
        if conn.initialized:
            return
        self._acp_request(
            conn,
    ...
    def _open_new_session(
        self, conn: "_AcpConnection", model: str | None, timeout_seconds: float, disable_tools: bool = False
    ) -> str:
        """Open a fresh ACP session, bind the model, return its sessionId."""
        params: dict[str, Any] = {"cwd": conn.cwd, "mcpServers": []}
        if disable_tools:
            # Hermes drives tools via <tool_call> text; drop the engine's own tools
            # (shrinks the system prefix + keeps file ops under Hermes governance).
            params["_meta"] = {"disableBuiltInTools": True}
        session = self._acp_request(
            conn, "session/new", params, timeout_seconds=timeout_seconds
        ) or {}
        session_id = str(session.get("sessionId") or "").strip()
        if not session_id:
            raise RuntimeError("Copilot ACP did not return a sessionId.")
        # Bind the requested model once at session start — never switched
    ...
    def _send_prompt(
        ...
        """Send one ``session/prompt`` on an (existing) session, collect text +
